...
if ( class_exists( 'TribeEvents' ) ) {
	...
	function tribe_events_event_schedule_details( $event = null, $before = '', $after = '' ) {
		...
		$datetime_separator       = tribe_get_option( 'dateTimeSeparator', ' <br/> <strong> Börjar: </strong> ' );
		$time_range_separator     = tribe_get_option( 'timeRangeSeparator', '  <br/> <strong> Slutar: </strong>  ' );
		...
		if ( tribe_event_is_multiday( $event ) ) { // multi-date event
			...
			if ( tribe_event_is_all_day( $event ) ) {
				$schedule .= ' <strong> Datum: </strong> '.tribe_get_start_date( $event, true, $format );
				...
				$schedule .= ' <strong> Datum: </strong> '.tribe_get_start_date( $event, false, $format ) . ( $time ? $datetime_separator . tribe_get_start_date( $event, false, $time_format ) : '' );
				$schedule .= '<span class="value-title" title="' . $microformatStartFormat . '"></span>';
				$schedule .= '</span>' . $time_range_separator;
				$schedule .= '<span class="date-end dtend">';
			...
			if ( tribe_get_start_date( $event, false, 'g:i A' ) === tribe_get_end_date( $event, false, 'g:i A' ) ) { // Same start/end time
				...
				$schedule .= ' <br/>' ;
				  $mykey_values = get_the_title(tribe_get_venue_id( $event ));
   
    $schedule .=  "<strong>Plats: </strong>$mykey_values<br />"; 
